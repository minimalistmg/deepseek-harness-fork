...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-voice': { kind: 'none', reason: 'Browser-side dictation; a transcript is ordinary draft text until the user sends it, and its provider calls sit outside ctx.llm.' },
  'packages/client/ui-holds': { kind: 'none', reason: 'Browser-side parked drafts; a held draft reaches a model only through the ordinary composer submission that sends it.' },
  'packages/client/ui-balance': { kind: 'none', reason: 'Browser-side account reading over the platform API; it registers no prompt, schema, or session event.' },
